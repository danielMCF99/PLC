...
Variavel : ID                                          { if(erro == 0){ declara($1,0); acc = asprintf(&$$, "PUSHI 0\n"); } }
...
Atrib : ID '<''-' ExprR ';'                                  { if(erro == 0) { posicao = existe($1); if(posicao != -1) { lista[posicao].valor = 1; acc = asprintf(&$$,"%sSTOREG %d\n",$4,posicao); } } }
      | ID '<' ExprR '>' '<''-' ExprR ';'                    { if(erro == 0) { posicao = existe($1); if(posicao != -1) { lista[posicao].valor = 1; acc = asprintf(&$$,"PUSHGP \nPUSHI %d\n%sADD\n%sSTOREN\n",posicao ,$3 ,$7); } } }
      ;
...
Fator : NUM                                 { if(erro == 0) { acc = asprintf(&$$,"PUSHI %d\n",$1); } }
      | '-' NUM                             { if(erro == 0) { acc = asprintf(&$$,"PUSHI %d\n",-1 * $2); } }
      | ID                                  { if(erro == 0) { posicao = existe($1); if(posicao >= 0){ acc = asprintf(&$$,"PUSHG %d\n",posicao); } 
                                                                                    else{ erro = 1; yyerror("tentou usar uma variavel que nao esta declarada ou nao tem um valor atribuido");} } } 
      | ID '<' NUM '>'                      { if(erro == 0) { posicao = existe($1); if(posicao >= 0){ acc = asprintf(&$$,"PUSHG %d\n",posicao + $3); }
                                                                                    else{ erro = 1; yyerror("tentou usar uma variavel que nao esta declarada ou nao tem um valor atribuido");} } } 
      | ID '<' ID '>'                       { if(erro == 0) { posicao = existe($1); int pos2 = existe($3); if(posicao >= 0 && pos2 >= 0){ acc = asprintf(&$$,"PUSHGP\nPUSHI %d\nPUSHG %d\nADD\nLOADN\n",posicao,pos2); }
                                                                                    else{ erro = 1; yyerror("tentou usar uma variavel que nao esta declarada ou nao tem um valor atribuido");} } } 
