...
Atrib : ID '<''-' ExprR ';'                                  { if(erro == 0) { posicao = existe($1); if(posicao != -1) { atribINT($1,$4,posicao); calculado = 0; }; }; }
      | ID '<' ExprR '>' '<''-' ExprR ';'                    { if(erro == 0) { posicao = existe($1); if(posicao != -1) { atribINT($1,$7,posicao+$3); calculado = 0; }; }; }
      ;
...
Condicao : ExprR                           { if(erro == 0) { $$ = $1; contaIF++; fprintf(fp,"%s %s%d\n","jz","ELSE",contaIF); }; }
